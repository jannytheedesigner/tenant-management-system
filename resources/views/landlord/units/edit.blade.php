<x-app-layout>
    <div class="p-6 max-w-3xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">
            Edit Unit - {{ $unit->unit_number }} ({{ $property->name }})
        </h1>

        <form action="{{ route('units.update', [$property->id, $unit->id]) }}" method="POST"
            ...
            <div>
                ...
                <input type="number" id="rent_amount" name="rent_amount"
                       value="{{ old('rent_amount', $unit->rent_amount) }}" step="0.01"
                       class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500"
                       required>
                @error('rent_amount')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
